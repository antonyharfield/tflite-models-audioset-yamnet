...
def _naive_rdft(signal_tensor, fft_length, padding='center'):
    ...
    complex_dft_matrix_kept_values = _dft_matrix(
        fft_length)[:(fft_length // 2 + 1), :].transpose()
    real_dft_tensor = tf.constant(np.real(complex_dft_matrix_kept_values).astype(
        np.float32), name='real_dft_matrix')
    imag_dft_tensor = tf.constant(np.imag(complex_dft_matrix_kept_values).astype(
        np.float32), name='imaginary_dft_matrix')
...
def _fixed_frame(signal, frame_length, frame_step, first_axis=False):
    ...
    if length_samples <= 0:
        raise ValueError(
            'fixed framing requires predefined constant signal length')
    ...
    subframe_length = fractions.gcd(
        frame_length, frame_step)  # pylint: disable=deprecated-method
    subframes_per_frame = frame_length // subframe_length
    subframes_per_hop = frame_step // subframe_length
    num_subframes = length_samples // subframe_length
    if first_axis:
        trimmed_input_size = [num_subframes *
                              subframe_length] + inner_dimensions
        subframe_shape = [num_subframes, subframe_length] + inner_dimensions
    else:
        trimmed_input_size = outer_dimensions + \
    ...
    subframe_selector = np.reshape(np.arange(subframes_per_frame), [
    ...
    frames = tf.reshape(tf.gather(subframes, selector.astype(
        np.int32), axis=gather_axis), result_shape)
    return frames
...
def _stft_magnitude_tflite(waveform_input, window_length_samples, hop_length_samples, fft_length):
    ...
    real_stft, imag_stft = _stft_tflite(
        waveform_input, frame_length=window_length_samples, frame_step=hop_length_samples, fft_length=fft_length)
    stft_magnitude = tf.sqrt(tf.add(real_stft * real_stft,
                                    imag_stft * imag_stft), name='magnitude_spectrogram')
...
def _stft_magnitude_full_tf(waveform_input, window_length_samples, hop_length_samples, fft_length):
    ...
    sftfs = tf.signal.stft(waveform_input, frame_length=window_length_samples,
...
def waveform_to_log_mel_spectrogram(waveform, params):
    ...
    with tf.name_scope('log_mel_features'):
        ...
        spectrogram_abs = _stft_magnitude_tflite(
            waveform_input=waveform, window_length_samples=window_length_samples, hop_length_samples=hop_length_samples, fft_length=fft_length)
        ...
        mel_spectrogram = tf.matmul(
            spectrogram_abs, linear_to_mel_weight_matrix)
...
def yamnet_frames_model(feature_params):
    """Defines the YAMNet waveform-to-class-scores model (suitable for tflite)

    Args:
      feature_params: An object with parameter fields to control the feature
      calculation.
      num_samples: A fixed length of samples to generate a fixed input model.
    ...
    spectrogram = tf.squeeze(waveform_to_log_mel_spectrogram(
        waveform, feature_params), axis=0)
